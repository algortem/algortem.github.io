...
var FILL_MODE = false
var COLORS = ['rgb(255, 255, 255)', 'rgb(255, 102, 46)', 'rgb(26, 218, 84)', 'rgb(83, 15, 255)', 'rgb(255, 236, 26)', 'rgb(142, 229, 255)']
...
// Выбор цвета
let color_cells = document.querySelectorAll('.page11 .color-cell')
color_cells.forEach(color_cell => {
    color_cell.addEventListener('click', function() {
        ...
        document.querySelector('.page11 .selected').classList.remove('selected')
        color_cell.classList.add('selected')
    })
})

// Отдельный обработчик для ластика и заливки, потому что это не color-cell, а tool-cell.
document.querySelector('.page11 .eraser').addEventListener('click', function() {
...
document.querySelector('.page11 .fill-tool').addEventListener('click', function() {
    FILL_MODE = !FILL_MODE
    document.querySelector('.page11 .selected').classList.remove('selected')
    this.classList.add('selected')
})
...
setInterval(function() {
    result = ''
    let temp_cells = document.querySelectorAll('.page11 .cell')
